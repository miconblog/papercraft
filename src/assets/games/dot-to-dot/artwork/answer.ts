 ...
 * 나와야 한다.
 */
import { fitOutline, toPoints } from '../../../../lib/dot-to-dot/index.ts';
import {
  ANSWER,
  ANSWER_BOX,
  ANSWER_STROKE_MM,
...
export const renderAnswer = (input: AnswerInput): string => {
  ...
  const fitted = fitOutline(toPoints(input.outline), ANSWER_BOX);
  ...
  return svgDocument({
    ...
    children: [
      ...
      group({ id: ART_LAYER_ID, fill: INK_COLOR, stroke: 'none' }, [
        path(outlinePath(fitted.flatMap((p) => [p.x, p.y])), {
          fill: 'none',
          stroke: INK_COLOR,
...
export const renderDefaultAnswer = (): string =>
  renderAnswer({ outline: SAMPLE_OUTLINE });
