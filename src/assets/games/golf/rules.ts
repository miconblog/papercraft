...
export const RULES: readonly RuleBlock[] = [
  ...
  {
    kind: 'bullet',
    text: `뽑을 것은 홀 판 ${HOLES.length}장(1번 홀부터 18번 홀까지)과 기록표 1장이다. 오리거나 접을 것은 없다. 판은 한 번에 다 뽑아도 되고, 칠 홀만 뽑아도 된다.`,
  },
  {
    kind: 'bullet',
    text: '「오늘의 라운드」는 **사람마다 한 장씩** 뽑는다. 그래프 선이 하나라 여럿이 한 장을 나눠 쓸 수 없다. 없어도 골프는 칠 수 있다 — 숫자를 눈으로 보고 싶을 때 뽑는다.',
  },
  ...
  {
    kind: 'step',
    text: '「오늘의 라운드」를 뽑았다면, 홀이 끝날 때마다 그 홀까지의 누적 파차에 점을 찍고 앞 점과 잇는다. 라운드가 끝나면 방금 낸 두 값을 아래 눈금자 둘에 찍는다 — 0의 왼쪽이 어느 쪽인지 눈으로 보인다.',
  },
