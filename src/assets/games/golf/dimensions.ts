 ...
 * 나오고, 거기서 코스 파(72)를 빼면 오늘의 성적이 된다. 지난 라운드의 총타수를
 * 한 번 더 빼면 지난번보다 얼마나 나아졌는지가 나온다(IDE-042) — 덧셈 한 번,
 * 뺄셈 두 번이 종이 위에 그대로 남는다.
 */
export const SCORE_CARD = {
  ...
  /**
   * 합산 칸 — 전반 + 후반 = 총타수, 총타수 − 파 = 성적, 총타수 − 지난 = 나아진 만큼.
   *
   * 머리 줄이 8→11mm로 커진 것은 **부호 방향을 적기 위해서**다(IDE-042). 그만큼
   * 위(162→159mm)로 올려 아래 밑줄 안내(207.5mm)를 침범하지 않게 한다 —
   * 세로는 이 종이에서 가장 모자란 것이다.
   */
  sumBoxYMm: 159,
  sumHeaderRowMm: 11,
  sumRowMm: 8.5,
  sumFontMm: 3.4,
  /**
   * 머리 칸 둘째 줄 — "+면 오버파 · −면 언더파".
   *
   * 이 종이에서 **가장 작은 글자**다(밑줄 안내 2.8mm보다 작다). 그래서 회색이
   * 아니라 먹빛으로 찍는다 — 칸 이름보다 흐리면 정작 읽어야 할 말이 안 읽힌다.
   * ⚠︎ 배율 하한 0.85에서 2.2mm가 되므로 실물 출력으로 재확인이 필요하다.
   */
  sumSubFontMm: 2.6,
  sumSignFontMm: 5,
  /**
   * 손으로 쓰는 네모 (IDE-042).
   *
   * 견주기 칸은 인쇄된 식과 빈 자리가 섞여 있어, 자리가 비어 있기만 해서는
   * 어디에 쓰는지 알 수 없다. 높이 5.5mm는 줄 높이 8.5mm 안에서 위아래
   * 1.5mm를 남긴다 — 연필이 칸 선을 건드리지 않는다.
   */
  writeBoxWidthMm: 15,
  writeBoxHeightMm: 5.5,
...
/**
 * 합산 칸의 세로 경계 —
 * [이름][전반][+][후반][=][총타수][파와의 차이][지난 라운드][지난번과의 차이].
 *
 * 부호 칸(`+`·`=`)을 따로 둔 것은 **식으로 읽히게** 하기 위해서다. 칸 사이에
 * 부호가 인쇄되어 있으면 아이가 빈칸 셋을 채우는 동안 그것이 덧셈이라는 것을
 * 잊지 않는다.
 *
 * 마지막 열이 IDE-042에서 붙었다. **파 72로는 음수를 볼 수 없기 때문**이다 —
 * 실제 라운드에서 아이는 +27, 아빠는 +34였고, 언더파는 실력이 한참 늘기 전까지
 * 나오지 않는다. 지난 라운드를 0으로 삼으면 `94 − 99 = −5`처럼 음수가 실제로
 * 나오고, 좋아질수록 작아지므로 **음수가 좋은 것**이 된다.
 *
 * 견주기 두 칸이 넓은 것은 **칸 안에서 식이 완결되기 때문**이다 —
 * `▢ − 72 = ▢`, `▢ − ▢ = ▢`. 네모가 없으면 빈 자리만 남아 어디에 쓰는지
 * 알 수 없다(2026-09-20 사용자 요청). 지난 라운드의 총타수도 그 네모 안에
 * 들어가므로 따로 열을 두지 않는다.
 *
 * 합이 `SCORE_TABLE_WIDTH_MM`(267)와 같아야 한다 — 두 표와 세로선이 맞아야
 * 한 장으로 읽힌다. 자리는 이름 칸을 56→40mm로 줄여 만들었다(이름은 다섯 자가
 * 최대라 40mm로 남는다).
 */
export const SUM_COLUMNS_MM: readonly number[] = [
