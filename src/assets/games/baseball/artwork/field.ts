 ...
 *
 * ## 판 위에 남는 것 — S·O 카운터 하나
 *
 * 타순표와 아웃·주자 표시칸을 파울 지역에 두었다가 뺐고(2026-09-08 사용자
 * 요청 — "일단 없애고"), 그 대신 왼쪽 아래에 **스트라이크·아웃 카운터**를
 * 넣었다(같은 날 요청). 처음에는 볼 칸도 있었으나 실제로 해 보고 뺐다(같은 날) —
 * 남은 것은 S 둘 · O 둘뿐이다. 주자는 베이스 위에 동전을 얹어 표시한다(`../rules.ts`).
...
/**
 * 스트라이크·아웃 카운터 — 왼쪽 아래 파울 지역.
 *
 * 줄마다 글자 하나와 동그라미 둘이다(S 둘 · O 둘). 셋째는 그리지 않는다 —
 * 스트라이크 셋째는 아웃이고 아웃 셋째는 공수 교대라 그 순간 칸이 비워진다
 * (`../dimensions.ts`의 `COUNT_PANEL`). 동전을 얹어 센다. 글자는 먹, 동그라미는
 * 표 색(회색)이다 — 판정선과 같은 색을 쓰면 선이 하나 더 있는 것처럼 보인다.
 */
