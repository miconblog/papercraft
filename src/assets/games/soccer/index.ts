 ...
 * 점수 기록칸 헤더는 **왼쪽 첫 칸의 사선("이름 \ 판") 하나뿐**이고 나머지는
 * 비어 있다 — 등번호처럼 아이가 종이에 직접 쓰는 자리다. 어느 칸이 어느 팀인지도
 * 그 손글씨가 정한다(2026-09-08 사용자 요청으로 색 막대를 뺐다).
 */
  ...
  parts: [
    ...
    },
    {
      ...
      description:
        ...
        '풀도 칼도 쓰지 않는다 — 뒷벽 탭과 뚜껑 귀를 옆벽 안쪽에 대고 겹으로 ' +
        '한꺼번에 물린다. 앞으로 뻗은 입술이 운동장 위에 얹혀 공이 넘을 턱이 없고, ' +
        '뚜껑이 위를 막아 골포스트를 맞고 튄 공도 밖으로 나가지 않는다. 운동장 ' +
        '골라인의 눈금에 골대 앞 모서리 좌우 끝을 맞춰 놓는다. **한 장에 두 개**가 들어 있다 — ' +
