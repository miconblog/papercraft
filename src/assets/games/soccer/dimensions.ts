...
/**
 * 종이 골대 — 오려 접어 골라인 앞에 놓는 **뚜껑 없는 쟁반**.
 *
 * 모양은 쓰레받기(사용자 표현으로 "국자")다. 바닥 하나, 벽 셋(뒤·좌·우), 위는
 * 뚫려 있고, 앞으로 납작한 **입술**이 뻗어 나온다. 공은 운동장에서 입술을 타고
 * 굴러 들어와 쟁반 안에 멈추고, 위가 뚫려 있으니 들어간 공이 그대로 보인다.
 *
 * 구조가 네 번 바뀌었다. 전부 놀아 보고 나온 문제를 고친 것이다:
 *
 ...
 * 4. 지금은 **뚜껑 없는 쟁반**이다(2026-09-06 사용자 요청). 지붕이 없으니
 *    창도, 풀칠탭도, 칼집도 필요 없다 — 위에서 그냥 들여다보면 된다.
 *
...
export const GOAL = {
  ...
  mouthWidthMm: 70,
  /**
   * 벽 셋의 높이. 앞이 뚫려 있으므로 이 값이 곧 **골문 높이**다.
   *
   * 지름 12mm 공의 두 배다. 낮으면 굴러 들어온 공이 뒷벽을 타고 튀어 나가고,
   * 높으면 위에서 안이 안 보인다.
   */
  wallHeightMm: 24,
  /**
...
export const GOAL_NET_SIZE = {
  ...
  heightMm: GOAL.wallHeightMm + GOAL.trayDepthMm + GOAL.lipDepthMm, // 76
} as const;
...
export const SHEETS = {
  ...
  /**
   * 골대 전개도 2벌 + 조립 안내 — **A4 가로 한 장에 딱 들어가는 크기**.
   *
   * A5(210×148.5)였다. 골문이 사진 크기(70×54)로 커지면서 전개도 한 벌이
   * 122×92mm가 되어 두 벌을 A5에 나란히 놓을 수 없다.
   *
   * 그렇다고 운동장처럼 297×210(=A4 그 자체)으로 두면 **한 장에 안 들어간다**.
   * 인쇄 가능 영역은 여백 6mm를 뺀 285×198이라 297×210짜리 파트는 두 장으로
   * 쪼개져 이어 붙여야 한다(운동장이 A4 2장인 이유가 이것이다). 조립물을 이어
   * 붙이게 하면 전개도가 이음매에서 어긋난다.
   *
   * 그래서 285×198보다 조금 작은 280×190으로 잡는다. 남는 5×8mm 가운데 세로
   * 여유가 하단 표식 띠(7mm) 자리다 — 띠가 도안을 덮지 않는다.
   */
