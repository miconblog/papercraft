...
export const GOAL = {
   ...
   *
   * 8 → 12mm로 늘렸다. 겹(8mm)에 물리고도 4mm가 남아야 겹 밑으로 조금 들어가
   * 확실히 잡힌다 — 겹과 길이가 똑같으면 1mm만 어긋나도 물리지 않는다.
   */
  lidFlapMm: 12,
  /**
   * 귀가 경첩(뒷벽)에서 얼마나 물러나 앉는지.
   *
   * 귀와 모서리 탭은 접고 나면 **둘 다 옆벽 안쪽 면**에 붙는다. 탭이 뒷벽에서
   * 앞으로 `cornerTabMm`만큼 뻗으므로, 귀는 그만큼 물러나야 자리를 다투지 않는다
   * — 겹치면 그 자리만 종이가 넉 겹이 되어 겹이 안 닫힌다.
   *
   * `cornerTabMm`과 같은 값이면 딱 맞닿는다. 테스트가 `귀 물러남 ≥ 탭 길이`를
   * 지킨다.
   */
  lidEarInsetMm: 10,
  /**
...
export const GOAL_NET_SIZE = {
  ...
  heightMm:
    GOAL.lidDepthMm + GOAL.wallHeightMm + GOAL.trayDepthMm + GOAL.lipDepthMm, // 100
} as const;
...
export const SHEETS = {
   ...
   *
   * 280×176은 **여백 8mm까지 한 장을 지킨다**(폭이 한계다). 남는 자리는 하단
   * 표식 띠(7mm) 몫으로도 쓰인다 — 띠가 도안을 덮지 않는다.
   */
  goals: { widthMm: 280, heightMm: 176 },
} as const;
