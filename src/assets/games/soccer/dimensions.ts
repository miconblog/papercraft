...
export const GOAL = {
   ...
   * 지나가야 해서 과장했다(`지름 × 2 < 폭`을 테스트가 지킨다).
   */
  mouthWidthMm: 39,
  /** 크로스바 아래 높이. 39:13 = **3:1**로 실제 골대의 가로세로비와 같다. */
  mouthHeightMm: 13,
  /**
   * 골라인에서 뒤로 나가는 깊이 = 옆벽·지붕의 깊이.
   *
   * 지름 12mm 공이 온전히 들어가고도 남아야 "골대 안에 멈췄다"가 한눈에
   * 보인다. 골라인 바깥 여백은 `FIELD.xMm`(6mm)뿐이라 나머지 8mm는 종이 밖
...
export const GOAL_ROOF_WINDOW_SIZE = {
  widthMm: GOAL.mouthWidthMm - GOAL_ROOF_WINDOW.sideBarMm * 2, // 31
  depthMm:
    GOAL.depthMm - GOAL_ROOF_WINDOW.frontBarMm - GOAL_ROOF_WINDOW.backBarMm, // 7
} as const;

/** 전개도 한 벌의 외곽 크기. `artwork/goals.ts`가 이 값으로 배치한다. */
export const GOAL_NET_SIZE = {
  widthMm: GOAL.depthMm * 2 + GOAL.mouthWidthMm, // 67
  heightMm: GOAL.depthMm + GOAL.mouthHeightMm + GOAL.footDepthMm, // 34
} as const;
 ...
 *
 * 값을 2026-09-05에 다시 잡았다. 마커를 15 → 24mm로 키우려면 레인 사이 최소
 * 간격이 17mm로는 모자랐다 — 지금은 **26mm**다. 간격 넷이 대칭이라
 * (`d−g`, `297−f−d`, `f−a`, `m+a−297`, `297−2m`) 이 다섯 값만 26 이상이면
 * 열 레인 전체가 안전하다.
 */
export const FORMATION_LANES = {
   ...
   *
   * 골대가 골라인 안쪽을 차지하던 때는 그 자리를 피하느라 30이었다. 골대가
   * 골라인 위로 올라가면서 골문 앞으로 당겨 왔고, 마커가 24mm로 커지면서
   * 다시 조금 밀렸다 — `19 + 12 = 31`이 골 에어리어 끝(32) 안이다.
   */
  goalkeeper: 19,
  /** 수비 — 자기 진영(첫째 1/3). */
  defence: 45,
