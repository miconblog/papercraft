...
describe('게임 소개·게임 방법 페이지', () => {
  ...
  it('축구 게임판 소개를 렌더링한다 — 제목·인원·구성·뒤로가기', async () => {
    ...
    // 2026-09-12 사용자 요청 — 여기는 읽는 자리다. 히어로 도안 그림도
    // 만들기 버튼도 없고, 좌상단은 왔던 곳으로 돌아간다.
    expect(screen.getByRole('link', { name: /뒤로/ })).toHaveAttribute(
      'href',
      '/',
    );
    expect(screen.queryByRole('link', { name: /만들기/ })).toBeNull();
   ...
   */
  it('윷놀이 소개가 열린다 — 파트 셋과 게임 방법 절이 그대로 나온다', async () => {
    ...
    expect(screen.getByRole('link', { name: /뒤로/ })).toBeInTheDocument();
  });
