...
import { toMask, type Mask, type RgbaImage } from './image.ts';
import {
  chaikin,
  largestContour,
  simplify,
...
export type OutlineResult =
  | { readonly ok: true; readonly outline: FlatOutline }
  | {
      readonly ok: false;
...
const fail = (reason: OutlineFailure): OutlineResult => ({
  ok: false,
  reason,
...
export interface TraceOptions {
  ...
  /** 임계값을 직접 준다(`IDE-020`의 슬라이더). 없으면 Otsu가 정한다. */
